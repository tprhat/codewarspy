...
#
# def fibonacci(n): if n in [0, 1]: return n return fibonacci(n - 1) + fibonacci(n - 2) This algorithm serves welll
# its educative purpose but it's tremendously inefficient, not only because of recursion, but because we invoke the
# fibonacci function twice, and the right branch of recursion (i.e. fibonacci(n-2)) recalculates all the Fibonacci
# numbers already calculated by the left branch (i.e. fibonacci(n-1)).
#
# This algorithm is so inefficient that the time to calculate any Fibonacci number over 50 is simply too much. You
# may go for a cup of coffee or go take a nap while you wait for the answer. But if you try it here in Code Wars you
...
# we first look it up in the cache, if it's not there, we calculate it and put it in the cache, otherwise we returned
# the cached number.
#
